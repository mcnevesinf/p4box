...
monitor getInitialState(inout p4boxState pstate) on MyIngress {
	before {
...
/* Waypoint */
monitor parseWaypointLabel(inout p4boxState pstate) on MyParser{
	after parse_udp {
		state start {
...
monitor checkWaypoint(inout p4boxState pstate) on MyIngress {
	local{
...
monitor well_formedness(inout p4boxState pstate) on MyEgress {
	after{
...
monitor header_protection(inout p4boxState pstate) on MyEgress {
	after{
...
monitor traffic_locality(inout p4boxState pstate) on MyEgress {
	local{
